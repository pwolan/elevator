...
class Elevator {
    ...
    constructor(id: number, cells: HTMLDivElement[][], conf: config, private queues: FloorQueue[], cellsOnTheLeft: number) {
        this.id = id;
        this.capacity = conf.capacity
        this.column = cells.map(row => row[3 * id + (cellsOnTheLeft - 2)]).reverse()
